...
export async function savePayrollAction(formData: FormData) {
  ...
  try {
    ...
    await savePayrollRecord({ adminUserId, payrollMonth: month, baseSalary: amount(formData, "baseSalary"), payableAmount: amount(formData, "payableAmount"), workDays: amount(formData, "workDays"), status: value(formData, "status"), note: value(formData, "note") });
  } catch (error) {
    redirect(`/admin/payroll?month=${encodeURIComponent(month)}&error=${encodeURIComponent(error instanceof Error ? error.message : "Không thể lưu kiểm kê lương.")}`);
  }
  revalidatePath("/admin/payroll");
  redirect(`/admin/payroll?month=${encodeURIComponent(month)}&status=saved`);
}
